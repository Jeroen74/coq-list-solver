...
let mycc_printer_tac depth l = 
  Proofview.Goal.enter begin fun gl ->
    let state = make_prb gl depth l false in 
    mycc_printer state; Tacticals.tclIDTAC
  end

let mycc_printer_tac_from_to depth n m l = 
  Proofview.Goal.enter begin fun gl ->
    let state = make_prb gl depth l false in 
    mycc_printer_from_to state n m; Tacticals.tclIDTAC
  end

let make_eq env sigma lhs rhs = 
  let ty = (Retyping.get_type_of env sigma lhs) in
  let eq = (EConstr.mkApp (EConstr.of_constr (Globnames.printable_constr_of_global Coqlib.(lib_ref "core.eq.type")) , ([| ty ; lhs ; rhs |]))) in 
  eq

let rebuild_term env sigma aterm =
  let trm = ATerm.constr aterm in trm

let set_to_list set = 
  let f = begin fun elt list ->
    List.cons elt list
  end in
  Int.Set.fold f set []

let get_equivalence_constr_list depth a l = 
  Proofview.Goal.enter_one begin fun gl ->
    let env = Tacmach.pf_env gl in 
    let sigma = Tacmach.project gl in
    let state = make_prb gl depth (a :: l) false in
    let new_state = execute_ret true state in
    let int_of_constr = add_aterm state (decompose_term env sigma a) in
    let rep_int = find (forest new_state) int_of_constr in
    let f = begin fun i -> 
      EConstr.of_constr (rebuild_term env sigma (get_aterm_of_node state i))
    end in
    Proofview.tclUNIT (List.map f (set_to_list (get_equivalence_class state rep_int)))
  end


let alt_make_prb gls depth additional_terms b l r =
  let open Tacmach in
  let env=pf_env gls in
  let sigma=project gls in
  let state = empty env sigma depth in
    ...
    List.iter
      (fun decl ->
         ...
         begin
           let cid=Constr.mkVar id in
           match litteral_of_constr b env sigma (NamedDecl.get_type decl) with
               `Eq (t,a,b) -> alt_add_equality state cid a b l r
             | `Neq (t,a,b) -> add_disequality state (Hyp cid) a b
             | `Other ph ->
                 List.iter
                   (fun (cidn,nh) ->
...
let or_congruence_eq depth new_name e l =
  Proofview.Goal.enter begin fun gl ->
    let env = Tacmach.pf_env gl in
    let sigma = Tacmach.project gl in
    match litteral_of_constr false env sigma e with
        `Eq (t,left,right) -> 
          let state = alt_make_prb gl depth l false left right in
          let left_int = add_aterm state left in
          let right_int = add_aterm state right in
          let final_state = execute_ret true state in
          let left_rep_int = find (forest final_state) left_int in
          let right_rep_int = find (forest final_state) right_int in
          if left_rep_int = right_rep_int then
            Tacticals.tclIDTAC
          else
            let f = begin fun i -> 
              EConstr.of_constr (rebuild_term env sigma (get_aterm_of_node state i))
            end in
            let left_constr_list = List.map f (set_to_list (get_equivalence_class final_state left_rep_int)) in
            let right_constr_list = List.map f (set_to_list (get_equivalence_class final_state right_rep_int)) in
            let pair_candidates = cartesian left_constr_list right_constr_list in
            let f' = begin fun base_tac list_elem ->
              let (lhs,rhs) = list_elem in
              Tacticals.tclOR (assert_after (Name.mk_name new_name) (make_eq env sigma lhs rhs)) base_tac
            end in
            List.fold_left f' Tacticals.tclIDTAC pair_candidates
      | _ -> Tacticals.tclIDTAC
  end
