...
val alt_add_equality : state -> constr -> ATerm.t -> ATerm.t -> ATerm.t -> ATerm.t -> unit
...
val execute_ret : bool -> state -> state

val execute_ret_alt : bool -> state -> state

val pr_idx_term : Environ.env -> Evd.evar_map -> forest -> int -> Pp.t

val state_printer: state -> unit

val mycc_printer: state -> unit

val mycc_printer_from_to: state -> int -> int -> unit

val canonize_name : EConstr.t -> constr

val get_aterm_of_node : state -> int -> ATerm.t

val get_equivalence_class : state -> int -> Int.Set.t

val get_node_array_ints : state -> (int Array.t )

val clean_state_printer : state -> unit