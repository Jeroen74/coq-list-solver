...
let debug_congruence = CDebug.create ~name:"mycc" ()
...
module ST=struct
  ...
  let pair_printer b = 
    match b with
      (l, r) -> str "(" ++ int l ++ str ", " ++ int r ++ str ")"

  let print_table_pair a b =
    str "{sign: " ++ int b++ str "; pair:" ++ pair_printer a ++ str "}, "

  let int_sigtable_printer a b print_string =
    print_string ++ (print_table_pair a b)

  let sigtable_printer st = 
    IntPairTable.fold int_sigtable_printer st.toterm (str "")
...
type state =
    {uf: forest;
     ...
     combine: equality Stack.t;
     marks: (int * pa_mark) Stack.t;
            ...
            match ATerm.proj t' with
                Symb _ | Product (_,_) ->
                  let paf =
                    {fsym=b;
                     fnargs=0} in
                    Stack.add_seq state.marks (Seq.return (b,Fmark paf));
              ...
              | Constructor cinfo ->
                  ...
                  let pac =
                    {cnode= b;
                     ...
                     args=[]} in
                    Stack.add_seq state.marks (Seq.return (b,Cmark pac));
                    {clas=Rep (new_representative typ);
                     cpath= -1;
                     constructors=PacMap.empty;
                     vertex=Leaf;
...
let add_equality state c s' t' =
  ...
  let j = add_aterm state t' in
    Stack.add_seq state.combine (Seq.return {lhs=i;rhs=j;rule=Axiom(c,false)});
    Constrhash.add state.uf.axioms c (s',t')

let alt_add_equality state c s' t' l r =
  if (ATerm.equal s' l && ATerm.equal t' r) then 
    ()
  else if (ATerm.equal t' l && ATerm.equal s' r) then
    ()
    else
  let i = add_aterm state s' in
  let j = add_aterm state t' in
...
let union state i1 i2 eq=
  ...
  and r2= get_representative state.uf i2 in
    ...
    let f= Int.Set.union r1.fathers r2.fathers in
      ...
      PacMap.iter
        (fun pac b -> Stack.add_seq state.marks (Seq.return (b,Cmark pac)))
        state.uf.map.(i1).constructors;
      PafMap.iter
        (fun paf -> Int.Set.iter
           (fun b -> Stack.add_seq state.marks (Seq.return (b,Fmark paf))))
        r1.functions;
      match r1.inductive_status,r2.inductive_status with
        ...
        | Total (i,pac),Total _ -> Stack.add_seq state.marks (Seq.return (i,Cmark pac))
        | _,_ -> ()
...
let update t state = (* update 1 and 2 *)
  ...
  let rep = get_representative state.uf i in
    ...
    PacMap.iter
      (fun pac _ -> Stack.add_seq state.marks (Seq.return (t,Cmark (append_pac v pac))))
      (get_constructors state.uf i);
    PafMap.iter
      (fun paf _ -> Stack.add_seq state.marks (Seq.return (t,Fmark (fsucc paf))))
      rep.functions;
    try
      let s = ST.query sign state.sigtable in
        Stack.add_seq state.combine (Seq.return {lhs=t;rhs=s;rule=Congruence})
    with
              ...
              if n > 0 then
                match (oargs,args) with
                    s1::q1,s2::q2->
                      Stack.add_seq
                        state.combine
                        (Seq.return {lhs=s1;rhs=s2;rule=Injection(s,opac,t,pac,n)});
                      f (n-1) q1 q2
...
let one_step state =
    try
      let eq = Stack.pop state.combine in
        merge eq state;
        true
    with Stack.Empty ->
      try
        let (t,m) = Stack.pop state.marks in
...
let int_set_t_printer intset =
  ppintset_alt intset

let rec int_set_PacMap_t_printer constructors ppstring= 
match (PafMap.choose_opt constructors) with
    None -> ppstring 
    | Some (key,j) -> let new_string = ppstring ++ str "," ++ int_set_t_printer j in 
                        let new_cons = PafMap.remove key constructors in
                        int_set_PacMap_t_printer new_cons new_string
  

let rec int_list_printer ls =
  match ls with
      [] -> str ""
      |h::tl -> int h ++ str ", " ++ int_list_printer tl

let pac_printer pa =
    str "{ cnode: " ++ int pa.cnode ++ 
    str ", arity: " ++ int pa.arity ++
    str ", args: [ " ++ int_list_printer pa.args ++ str " ] }" 

let inductive_status_printer is = 
  match is with
    Unknown -> str "Unknown"
    | Partial pac -> str "Partial " ++ pac_printer pac
    | Partial_applied -> str "Partial_applied"
    | Total (i,pac) -> str "Total (" ++ int i ++ str ",  " ++ pac_printer pac ++ str ")"

let type_printer ctype =
    str "{TODO}"

let int_set_map_printer functions =
  int_set_PacMap_t_printer functions (str "")

let clean_rep_printer rep =
    str "( weight: " ++ int rep.weight
    ++ str ", lfathers: "  ++ int_set_t_printer rep.lfathers
    ++ str ", fathers: " ++ int_set_t_printer rep.fathers
    ++ str ", inductive_status: " ++ inductive_status_printer rep.inductive_status
    ++ str ", functions: { " ++ int_set_map_printer rep.functions
    ++ str " })"
...
let rep_printer rep =
    ...
    ++ str ", fathers: " ++ int_set_t_printer rep.fathers
    ++ str ", inductive_status: " ++ inductive_status_printer rep.inductive_status
    ++ str ", class_type: " ++ type_printer rep.class_type
    ++ str ", functions: { " ++ int_set_map_printer rep.functions
    ++ str " })"
...
let eq_printer eq =
    str "[lhs: " ++ int eq.lhs ++ str ", rhs: " ++ int eq.rhs ++
    match eq.rule with
        Congruence -> str ", Congruence]"
        | Axiom (a,b) -> str ", Axiom (" ++ axiom_printer a ++ str "," ++ bool b ++ str ")]"
        | Injection (i,pac1,j,pac2,k) -> str ", Injection (" ++ int i ++ str "," 
                                            ++ pac_printer pac1 ++ str "," ++ int j ++ str "," 
                                            ++ pac_printer pac2 ++ str "," ++ int k ++ str ")]"
let clean_eq_printer eq =
    str "[lhs: " ++ int eq.lhs ++ str ", rhs: " ++ int eq.rhs ++
    match eq.rule with
        Congruence -> str ", Congruence]"
        | Axiom (a,b) -> str ", Axiom]"
...
let rec int_PacMap_t_printer constructors ppstring= 
    match (PacMap.choose_opt constructors) with
        None -> ppstring 
        | Some (key,j) -> let new_string = ppstring ++ str "," ++ int j in 
                            let new_cons = PacMap.remove key constructors in
                            int_PacMap_t_printer new_cons new_string

let vertex_printer vertex = 
    match vertex with
        Leaf -> str "Leaf"
        | Node (i,j) -> str "Node (" ++ int i ++ str "," ++ int j ++ str ")"


let clean_clas_printer cl = 
    match cl with
        Rep rep -> str "Rep " ++ clean_rep_printer rep
        | Eqto (i,eq) -> str "Eqto (" ++ int i ++ str "," ++ clean_eq_printer eq ++ str ")"


let clas_printer cl = 
    match cl with
        Rep rep -> str "Rep " ++ rep_printer rep
        | Eqto (i,eq) -> str "Eqto (" ++ int i ++ str "," ++ eq_printer eq ++ str ")"

let term_printing env sigma uf i =
    try pr_idx_term env sigma uf i with _ -> str "error"

let clean_node_printer state index node = 
    try begin let _ = pr_idx_term state.env state.sigma state.uf index in ();
      let _ = Feedback.msg_notice begin
        str "{ vertex: " ++ term_printing state.env state.sigma state.uf index
        ++ str ", cpath: " ++ int node.cpath 
        ++ str ", clas" ++ clean_clas_printer node.clas ++ str "}" 
      end in 
      () end
    with _ -> ()

let node_printer state index node = 
    let _ = Feedback.msg_notice begin
      str "{ clas: " ++ clas_printer node.clas
      ++ str ", cpath: " ++ int node.cpath 
      ++ str ", constructors : [" ++ int_PacMap_t_printer node.constructors (str "") ++ str "]"
      ++ str ", vertex: " ++ vertex_printer node.vertex
      ++ str ", aterm: " ++ term_printing state.env state.sigma state.uf index ++ str "}" 
    end in 
    ()

let forest_printer state =
  let node_array = state.uf.map in
  Array.iteri (node_printer state) node_array

let clean_forest_printer state =
  let node_array = state.uf.map in
  Array.iteri (clean_node_printer state) node_array
  
let rec rec_equality_printer queue pp_string =
  try 
    let new_string = pp_string ++ eq_printer (Stack.pop queue) ++ str ", " in 
    rec_equality_printer queue new_string
  with
    Stack.Empty -> pp_string ++ str "end of list."

let equality_queue_printer combine =  
  rec_equality_printer combine (str "")

let fmark_printer mark = 
  str "{ fsym: " ++ int mark.fsym
  ++ str ", fnargs: " ++ int mark.fnargs ++ str " }"

let pa_mark_printer pa = 
  match pa with 
    Fmark mark -> str "Fmark: " ++ fmark_printer mark
    | Cmark mark -> str "Cmark: " ++ pac_printer mark

let int_pa_mark_queue_printer int_pa_mark = 
  match int_pa_mark with 
    (i,pa) -> str "(" ++ int i ++ str ", " ++ pa_mark_printer pa ++ str ")"

let rec rec_int_pa_mark_printer marks pp_string = 
  try
    let new_string = pp_string ++ int_pa_mark_queue_printer (Stack.pop marks) ++ str ", " in
    rec_int_pa_mark_printer marks new_string
  with
    Stack.Empty -> pp_string ++ str "end of list."

let int_pa_mark_queue_printer marks =
  rec_int_pa_mark_printer marks (str "")

let state_printer state =
    let _ = Feedback.msg_notice begin
      str "combine: " ++ equality_queue_printer (Stack.copy state.combine)
    end in 
    let _ = Feedback.msg_notice begin
      str "marks: " ++ int_pa_mark_queue_printer (Stack.copy state.marks)
    end in
    let _ = Feedback.msg_notice begin
      str "terms: " ++ int_set_t_printer state.terms
    end in 
    let _ = Feedback.msg_notice begin
      str "pa_classes: " ++ int_set_t_printer state.pa_classes
    end in 
    let _ = Feedback.msg_notice begin
      str "signature_table: " ++ ST.sigtable_printer state.sigtable
    end in
    let _ = Feedback.msg_notice begin
      str "forest: "
    end in 
    let _ = forest_printer state in ()

    let clean_state_printer state =
        let _ = Feedback.msg_notice begin
          str "combine: " ++ equality_queue_printer (Stack.copy state.combine)
        end in 
        let _ = Feedback.msg_notice begin
        ...
        let _ = clean_forest_printer state in () 

let mycc_printer state =
    (* state_printer state;
    let _ = Feedback.msg_notice (str " ") in *)
    let num_steps = (ref 1) in begin
      let _ = Feedback.msg_notice (str "Step " ++ int !num_steps ++ str "(Initial situation)") in begin
      num_steps := succ !num_steps; 
      clean_state_printer state end;
    let _ = Feedback.msg_notice (str "\n Taking step " ++ int !num_steps) in
    num_steps := succ !num_steps;
    while 
        Control.check_for_interrupt ();
        one_step state do 
        clean_state_printer state;
        let _ = Feedback.msg_notice (str "\n Taking step " ++ int !num_steps) in 
        num_steps := succ !num_steps;
        ()
    done;
    clean_state_printer state end
    (* let _ = Feedback.msg_notice (str "\n We are done, this is the complete overview of the last step" ++ str " ... ") in
    let _ = Feedback.msg_notice (str " ") in state_printer state *)

let mycc_printer_from_to state start_num end_num =
  (* state_printer state;
  let _ = Feedback.msg_notice (str " ") in *)
  let num_steps = (ref 1) in begin
    if (start_num <= !num_steps) && (!num_steps <= end_num) then begin
    let _ = Feedback.msg_notice (str "Step " ++ int !num_steps ++ str " (Initial situation)") in begin 
    clean_state_printer state end end
    else ();
    num_steps := succ !num_steps;
  while 
      Control.check_for_interrupt ();
      ...
      let _ = Feedback.msg_notice (str "\n Taking step " ++ int !num_steps) in 
      clean_state_printer state end
      else (let _ = Feedback.msg_notice (str "") in ());
      num_steps := succ !num_steps
  done
  end
  (* let _ = Feedback.msg_notice (str "\n We are done, this is the complete overview of the last step" ++ str " ... ") in
...
let rec execute_ret first_run state =
...
let rec execute_ret_alt first_run state =
  if state.rew_depth>0 then
    let l = find_instances state in 
      List.iter (add_inst state) l;
      if state.changed then
...
let rec execute first_run state =
  ...
  try
    while
      Control.check_for_interrupt ();
      one_step state do () (*clean_state_printer state*)
    done;
...
let get_aterm_of_node state i = state.uf.map.(i).aterm 

let get_equivalence_class state i = 
  let f = begin fun _ a b ->
    Int.Set.union a b
  end in 
  match state.uf.map.(i).clas with 
    Rep r -> PafMap.fold f r.functions Int.Set.empty
    | Eqto _ -> Int.Set.empty

let get_node_array_ints state = Array.mapi (fun i _ -> i) state.uf.map
